...
const MessagePreview = memo(({ message, highlight }: MessagePreviewProps) => {
  const messageInput = useMessageInput();
  const startReply = messageInput?.startReply;
  const startEdit = messageInput?.startEditing;
  ...
  const messageLink =
    message.channelType === "post"
      ? `/channels/${pathname.split("/")[2]}/${message.channelId}/${message.id}`
  ...
  const repliedToProfile = useProfilesStore((state) =>
    message.repliedToMessage
      ? state.getProfile(message.repliedToMessage.authorId)
      : null,
  );
  ...
  const hasEmotesOnly = useMemo(
    () => isOnlyEmojisAndWhitespace(message.content),
    [message.content],
  );
  const hasManyEmotes = useMemo(
    () =>
      hasEmotesOnly ? graphemeLengthWithoutSpaces(message.content) > 10 : false,
    [message.content, hasEmotesOnly],
  );
  return (
    <ContextMenu>
      ...
      <ContextMenuContent>
        ...
        {!isOptimistic && (
          <>
            {startReply && (
              <ContextMenuItem onClick={() => startReply(message)}>
                Reply
              </ContextMenuItem>
            )}
            <ContextMenuItem
              onClick={() => {
                navigator.clipboard.writeText(
                  `${process.env.NEXT_PUBLIC_APP_URL}${messageLink}`,
                );
              }}
            >
              Copy Link
