...
// AttachedWith Schema
export const AttachedWithSchema = z.enum(["message", "post"]);
...
export const UploadInitPayloadSchema = z
  .object({
    ...
    attachedWith: AttachedWithSchema.default("message"), // Default to message
  })
  .strict();
...
export const UploadInitialisedResponseSchema = z
  .object({
    attachmentId: z.string().trim().optional(), // Attachment ID (present if file already exists)
...
export const UploadCompletedResponseSchema = z
  .object({
    attachmentId: z.string().trim(), // Attachment ID
...
export type AttachedWith = z.infer<typeof AttachedWithSchema>;
